...
import { resolvePricing, type PricingEntry, type PricingTable } from './pricing.ts'
...
function zeroCounts(): ReceiptTokenCounts {
  return { inputTokens: 0, outputTokens: 0, cacheReadTokens: 0, cacheWriteTokens: 0, reasoningTokens: 0 }
}

function countsFrom(usage: TokenUsage): ReceiptTokenCounts {
  return {
...
function countsEqual(left: ReceiptTokenCounts, right: ReceiptTokenCounts): boolean {
...
function bucketKeyOf(sample: StepSample): string {
...
function isBucketEmpty(bucket: ModelBucketState): boolean {
...
function subtract(models: Record<string, ModelBucketState>, sample: StepSample): Record<string, ModelBucketState> {
  const key = bucketKeyOf(sample)
  const bucket = models[key]
  if (bucket === undefined) return models
  const next: ModelBucketState = {
    ...bucket,
  ...
  // 归零的桶（例如同一 step 的旧样本被新样本替换后）不留在 state 里。
...
function add(models: Record<string, ModelBucketState>, sample: StepSample): Record<string, ModelBucketState> {
  const key = bucketKeyOf(sample)
  const existing = models[key]
  const bucket: ModelBucketState = existing ?? {
 ...
 */
function replaceStep(state: ReceiptState, turn: number, step: number, next: StepSample): ReceiptState {
...
function undefinedSample(): StepSample {
...
function beijingHour(time: number): number {
...
export function receiptView(
  ...
  const totals = rows.reduce<ReceiptTokenCounts & { calls: number; cost: number; peakCost: number }>(
    (acc, row) => ({
...
// 本文件同时被 host tsc program 编译；`receipt` 键在折叠状态表中声明
// （wire 侧键已在 types.ts 的 SessionProjectionMap 中声明）。
...
export function receiptProjectionDefinition(
  pricing: PricingTable,
  currency: string,
  options: PeakPricingOptions = {},
): ProjectionDefinition<'receipt', ReceiptState> & {
